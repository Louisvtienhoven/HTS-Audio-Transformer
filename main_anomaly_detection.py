...
pretrained_ckpt = os.path.join(checkpoint_path, 'htsat_audioset_pretrain.ckpt')
if not os.path.exists(pretrained_ckpt):
    print("[INFO] Downloading pretrained checkpoint...")
    gdown.download(id='1OK8a5XuMVLyeVKF117L8pfxeZYdfSDZv', output=pretrained_ckpt)

print("[INFO] Pretrained checkpoint downloaded or already exists at", pretrained_ckpt)
# Load and process ESC-50 dataset
meta_path = os.path.join(esc_raw_path, 'ESC-Master', 'meta', 'meta_final_dataset_250.csv')
audio_path = os.path.join(esc_raw_path, 'ESC-Master', 'Dataset_final_1_second')
...
class DataPrep(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        return DataLoader(self.train_dataset, batch_size=config.batch_size // self.device_num, shuffle=(sampler is None), sampler=sampler, num_workers=config.num_workers)

    def val_dataloader(self):
        sampler = DistributedSampler(self.eval_dataset, shuffle=False) if self.device_num > 1 else None
        return DataLoader(self.eval_dataset, batch_size=config.batch_size // self.device_num, shuffle=False, sampler=sampler, num_workers=config.num_workers)
...
checkpoint_callback = ModelCheckpoint(monitor="acc", filename='l-{epoch:d}-{acc:.3f}', save_top_k=20, mode="max")
...
if config.resume_checkpoint is not None:
    ...
    for key in ["sed_model.head.weight", "sed_model.head.bias", "sed_model.tscam_conv.weight", "sed_model.tscam_conv.bias"]:
