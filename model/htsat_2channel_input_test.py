...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
                 in_chans=2, num_classes=527,  # <-- CHANGE: default in_chans to 2
        ...
        # CHANGE: BatchNorm2d for 2 channels (spectrogram+amplitude)
        self.bn0 = nn.BatchNorm2d(self.in_chans)
    ...
    def crop_wav(self, x, crop_size, spe_pos=None):
        ...
        for i in range(len(x)):
            if spe_pos is None:
                ...
                crop_pos = spe_pos
            tx[i, :, :, :] = x[i, :, crop_pos:crop_pos + crop_size, :]
        return tx
    ...
    def reshape_wav2img(self, x):
        """
        Resize any (B, C, T, F) tensor to (B, C, spec_size, spec_size)
        so it always fits the Swin patch embedding.
        """
    ...
    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):
        """
        x: (B, T) or (B, 1, T) waveform, float32
        Returns: output_dict
        """
        # 1. Compute linear spectrogram (magnitude)
        spec = self.spectrogram_extractor(x).float()  # (B,1,T_spec,F_spec)

        # 2. Stack spectrogram (and optionally raw amplitude) as channels
        if self.in_chans == 1:
            # only spectrogram
            x = spec  # (B,1,T_spec,F_spec)
        else:
            # compute & tile amplitude
            waveform = x if x.ndim == 2 else x[:, 0]  # (B, T)
            target_T = spec.shape[2]
            ...
            amplitude = amplitude.unsqueeze(3).repeat(
                1, 1, 1, spec.shape[3]
            )  # (B,1,T_spec,F_spec)
            x = torch.cat([spec, amplitude], dim=1)  # (B,2,T_spec,F_spec)

        # 3. BatchNorm + augmentation + mixup
        x = self.bn0(x)
        if self.training:
        ...
        # 4. Inference / repeat / cropping logic
        if infer_mode:
            frame_num = x.shape[2]
            target_T = int(self.spec_size * self.freq_ratio)
            repeat_ratio = math.floor(target_T / frame_num)
            x = x.repeat(1, 1, repeat_ratio, 1)
            ...
            if self.training:
                ...
                for cur_pos in range(0, (self.freq_ratio - 1) * self.spec_size + 1, self.spec_size):
                    ...
                    outputs.append(self.forward_features(tx))
                c = outputs[0]["clipwise_output"].clone().zero_()
                f = outputs[0]["framewise_output"].clone().zero_()
                for d in outputs:
                    c += d["clipwise_output"]
                    f += d["framewise_output"]
                output_dict = {
                    'clipwise_output': c / len(outputs),
                    'framewise_output': f / len(outputs)
                }
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                if self.training:
                    ...
                    for pos in range(0, x.shape[2] - crop_size - 1, overlap):
                        tx = self.crop_wav(x, crop_size=crop_size, spe_pos=pos)
