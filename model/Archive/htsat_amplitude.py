...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):  # out_feat_keys: List[str] = None):
        # Original logmel
        logmel = self.logmel_extractor(self.spectrogram_extractor(x))  # (B, 1, T, mel)

        # Compute amplitude (RMS) per time step, over the mel axis
        amplitude = torch.sqrt(torch.mean(logmel ** 2, dim=3, keepdim=True))  # (B, 1, T, 1)

        # Normalize amplitude for stability
        amplitude = (amplitude - amplitude.mean(dim=2, keepdim=True)) / (amplitude.std(dim=2, keepdim=True) + 1e-6)

        # Repeat across mel bins to match shape (B, 1, T, mel)
        amplitude = amplitude.repeat(1, 1, 1, logmel.shape[3])

        # Concatenate along the channel dimension
        x = torch.cat([logmel, amplitude], dim=1)  # Now shape: (B, 2, T, mel)
