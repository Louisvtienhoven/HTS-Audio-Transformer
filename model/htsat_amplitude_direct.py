...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def frame_rms(self, waveform, frame_length, hop_length):
        """Compute frame-wise RMS amplitude manually."""
        batch_size, signal_len = waveform.shape
        pad = (frame_length - hop_length) // 2
        waveform = torch.nn.functional.pad(waveform.unsqueeze(1), (pad, pad), mode='reflect')  # (B, 1, T)

        waveform = waveform.unfold(dimension=2, size=frame_length, step=hop_length)  # (B, 1, n_frames, frame_len)
        rms = torch.sqrt(torch.mean(waveform ** 2, dim=-1) + 1e-8)  # (B, 1, n_frames)
        return rms.squeeze(1)  # (B, n_frames)

    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):
        # Extract waveform and compute logmel
        logmel = self.logmel_extractor(self.spectrogram_extractor(x))  # (B, 1, T_logmel, mel)

        # Compute raw waveform amplitude (RMS)
        waveform = x if x.ndim == 2 else x[:, 0]  # (B, T)
        rms = self.frame_rms(waveform, frame_length=self.config.window_size, hop_length=self.config.hop_size)  # (B, T_rms)
        rms = (rms - rms.mean(dim=1, keepdim=True)) / (rms.std(dim=1, keepdim=True) + 1e-6)  # normalize

        # Interpolate RMS to match logmel T dimension
        target_T = logmel.shape[2]
        rms = F.interpolate(rms.unsqueeze(1), size=target_T, mode="linear", align_corners=True).squeeze(
            1)  # (B, T_logmel)

        # Reshape and expand to match (B, 1, T, mel)
        amplitude = rms.unsqueeze(1).unsqueeze(3).repeat(1, 1, 1, logmel.shape[3])

        # Combine logmel and true amplitude
        x = torch.cat([logmel, amplitude], dim=1)  # (B, 2, T, mel)
