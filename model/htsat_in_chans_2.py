...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):
        ...
        spec = self.spectrogram_extractor(x)  # (B, 1, T_spec, F_spec)
        spec = spec.float()

        # 2. Prepare raw amplitude per spectrogram frame
        waveform = x if x.ndim == 2 else x[:, 0]  # (B, T)
        target_T = spec.shape[2]
        # Interpolate the waveform to T_spec length (so 1 value per spectrogram frame)
        amplitude = torch.nn.functional.interpolate(
            waveform.unsqueeze(1), size=target_T, mode="linear", align_corners=True
        )  # (B, 1, T_spec)
        # Expand to match spectrogram shape
        amplitude = amplitude.unsqueeze(3).repeat(1, 1, 1, spec.shape[3])  # (B, 1, T_spec, F_spec)

        # 3. Stack spectrogram and raw amplitude as channels
