...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
        ...
        #Added
        self.mel_fb = self.create_mel_filterbank(
    ...
    @staticmethod
    def create_mel_filterbank(sample_rate, n_fft, n_mels, fmin, fmax):
        mel_fb_np = librosa.filters.mel(sr=sample_rate, n_fft=n_fft,
                                        n_mels=n_mels, fmin=fmin, fmax=fmax)
        mel_fb = torch.from_numpy(mel_fb_np).float()  # shape (n_mels, n_fft//2 + 1)
        return mel_fb
    ...
    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):  # out_feat_keys: List[str] = None):
        ...
        mel_fb = self.mel_fb.to(x.device)
        x = torch.matmul(x.squeeze(1), mel_fb.T).unsqueeze(1)  # (B, 1, T, mel_bins)
